import { CREATE_DOG, GET_DOGS, GET_NAME, GET_TEMPERAMENTS, FILTER, ORDER, ORDER_WEIGHT, FILTER_BD_API, } from "./actions-types";
    ...
    switch (action.type){
        ...
        case FILTER:
            ...
            const response = [...copyDogs.filter((dog)=>{
                return dog.temperamet && dog.temperament.split(',').map(element => element.trim()).includes(action.payload);
            })]
            return{
                ...state,
                newDogs: response
        ...
        case ORDER:
            if(action.payload === 'A'){
                ...
                const result = allDogsCopy.sort((a, b) => a.name.localCompare(b.name));
                return{
                    ...state,
                    newDogs: [...result]
                };
            ...
            if(action.payload === 'D'){
                const allDogsCopy = [...state.newDogs];
                const result = allDogsCopy.sort((a, b) => b.name.localCompare(a.name));
                return{
                    ...state,
                    newDogs: [...result]
                };
